...
function redrawGrid(property, value) {
	...
	$.get(globalHomePath+"Account/GetUserTable", { viewState: JSON.stringify(gridState) },
			function (data) {
				//success
				$("#userTable").html(data);
...
function deleteUser(id) {
	//ajax post to delete user
	$.post(globalHomePath+"Account/Remove", { "id": id },
			function (data) {
				//success
...
function storeTableParams() {
	sessionStorage.setItem('UserTable', JSON.stringify(gridState));
}
